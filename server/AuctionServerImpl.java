...
public class AuctionServerImpl extends UnicastRemoteObject implements IAuctionServer {
        ...
        public void run() {
            ...
            synchronized(this) {
                if ((expiredAuction = auctionItems.remove(id)) != null) {
                    ...
                    // Notify observers
                    StringBuilder message = new StringBuilder("Auction for item " + expiredAuction.getName() + " has ended. ");
                    
                    if (expiredAuction.getCurrentBid() != null) {
                        message.append("Winning bid - ").append(expiredAuction.getCurrentBid().getAmount());
                        message.append(" by ").append(expiredAuction.getCurrentBid().getBuyerName());
                    } else {
                        message.append("No winner!");
                    }

                    expiredAuction.notifyObservers(message.toString());
    ...
    @Override
    public String createAuctionItem(IAuctionSeller owner, String name, float minVal, long closingTime) throws RemoteException {
        if (owner == null)
            return ErrorCodes.OWNER_NULL.MESSAGE;

        LOGGER.info("Seller " + owner.getName() + " is trying to create an auction item");

        if (name == null)
            return ErrorCodes.ITEM_NAME_NULL.MESSAGE;

        if (name.length() == 0)
            return ErrorCodes.ITEM_NAME_EMPTY.MESSAGE;

        if (minVal < 0)
            return ErrorCodes.NEGATIVE_MINIMUM_VAL.MESSAGE;

        if (closingTime < 0)
            return ErrorCodes.NEGATIVE_CLOSING_TIME.MESSAGE;
    ...
    @Override
    public String bid(IAuctionBuyer buyer, int auctionItemId, float amount) throws RemoteException {
        if (buyer == null) return ErrorCodes.OWNER_NULL.MESSAGE;

        // Get buyer name so we don't have to query multiple times and risk a RemoteException
        String buyerName = buyer.getName();
        LOGGER.info("Client " + buyerName + " is trying to bid on item ID #" + auctionItemId);

        // Validate item exists and doesn't belong to the bidder
        AuctionItem item = auctionItems.get(auctionItemId);

        if (item == null)
            return ErrorCodes.AUCTION_DOES_NOT_EXIST.MESSAGE;

        if (item.getOwner() == buyer) 
            return ErrorCodes.BID_ON_OWN_ITEM.MESSAGE;

        // Make the bid (further validation is in Bid's constructor)
        Bid bid = new Bid(buyer, buyerName, amount);
        String result = item.makeBid(bid);

        LOGGER.info(result + " - " + buyer.getName() + ", item ID #" + auctionItemId);
        return result;
    }
    ...
    public String getClosedAuctions() throws RemoteException {
        if (closedAuctionItems.size() == 0) 
            return "No historical auctions";
